(function() {
  addLesson({
    level: "intermediate",
    lesson: "lesson10",
    name: "Урок 10: Future Ability with Will Be Able To",
    structures: [
      {
        structure: "Will you be able to ______________?",
        pattern: ["will", "you", "be", "able", "to"],
        translations: ["Сможешь ли ты ______?"],
        examples: [
          "Will you be able to come tomorrow? (Сможешь ли ты прийти завтра?)",
          "Will you be able to finish the work? (Сможешь ли ты закончить работу?)",
          "Will you be able to meet them later? (Сможешь ли ты встретиться с ними позже?)"
        ],
        id: "will-you-be-able-to-base",
        hasVerb: false,
        hasName: false
      },
      {
        structure: "Yes, I will be able to ________________.",
        pattern: ["yes", "i", "will"],
        translations: ["Да, я смогу ______."],
        examples: [
          "Yes, I will be able to come tomorrow. (Да, я смогу прийти завтра.)",
          "Yes, I will be able to finish the work. (Да, я смогу закончить работу.)",
          "Yes, I will be able to meet them later. (Да, я смогу встретиться с ними позже.)"
        ],
        id: "yes-i-will-be-able-to-base",
        hasVerb: false,
        hasName: false
      },
      {
        structure: "No, I will not be able to ______________.",
        pattern: ["no", "i", "will"],
        translations: ["Нет, я не смогу ______."],
        examples: [
          "No, I will not be able to come tomorrow. (Нет, я не смогу прийти завтра.)",
          "No, I will not be able to finish the work. (Нет, я не смогу закончить работу.)",
          "No, I will not be able to meet them later. (Нет, я не смогу встретиться с ними позже.)"
        ],
        id: "no-i-will-not-be-able-to-base",
        hasVerb: false,
        hasName: false
      }
    ],
    ...
    validateStructure: function(text, structure) {
      ...
      let minWords = structure.id === "will-you-be-able-to-base" ? 6 : 7; // 6 для вопроса, 7 для ответов
      if (structure.id === "no-i-will-not-be-able-to-base") minWords = 8; // 8 для отрицательного ответа
      ...
      const validAdverbs = ['hard', 'harder', 'well', 'better', 'fast', 'slowly', 'quickly', 'carefully', 'tomorrow', 'later', 'today'];
      ...
      if (structure.id === "will-you-be-able-to-base") {
        console.log('Начало проверки структуры Will you be able to base');
        const expectedStart = ['will', 'you', 'be', 'able', 'to'];
        ...
        return true;
      } else if (structure.id === "yes-i-will-be-able-to-base") {
        console.log('Начало проверки структуры Yes I will be able to base');
        const expectedStart = ['yes', 'i', 'will', 'be', 'able', 'to'];
        for (let i = 0; i < expectedStart.length; i++) {
          if (wordIndex >= words.length || words[wordIndex] !== expectedStart[i]) {
            console.log(`Ожидалось "${expectedStart[i]}" на позиции ${wordIndex}, получено`, words[wordIndex] || 'ничего');
            return false;
        ...
        console.log('Валидация пройдена для:', text);
        return true;
      } else if (structure.id === "no-i-will-not-be-able-to-base") {
        console.log('Начало проверки структуры No I will not be able to base');
        const expectedStart = ['no', 'i', 'will', 'not', 'be', 'able', 'to'];
        for (let i = 0; i < expectedStart.length; i++) {
          if (wordIndex >= words.length || words[wordIndex] !== expectedStart[i]) {
            console.log(`Ожидалось "${expectedStart[i]}" на позиции ${wordIndex}, получено`, words[wordIndex] || 'ничего');
            return false;
