(function() {
  addLesson({
    level: "intermediate",
    lesson: "lesson3",
    name: "Урок 3: Second Conditional and Wishes",
    structures: [
      {
        structure: "What would you do if you ______________(ed)?",
        pattern: ["what", "would", "you", "do", "if", "you"],
        translations: ["Что бы ты сделал, если бы ______?"],
        examples: [
          "What would you do if you won a prize? (Что бы ты сделал, если бы выиграл приз?)",
          "What would you do if you found money? (Что бы ты сделал, если бы нашел деньги?)",
          "What would you do if you got a job? (Что бы ты сделал, если бы получил работу?)"
        ],
        id: "what-would-you-do-if-you-past-simple",
        hasVerb: false,
        hasName: false
      },
      {
        structure: "If I ______________(ed), I would ___________.",
        pattern: ["if", "i"],
        translations: ["Если бы я ______, я бы ______."],
        examples: [
          "If I won a prize, I would travel the world. (Если бы я выиграл приз, я бы путешествовал по миру.)",
          "If I found money, I would buy a house. (Если бы я нашел деньги, я бы купил дом.)",
          "If I got a job, I would save money. (Если бы я получил работу, я бы копил деньги.)"
        ],
      ...
      {
        structure: "If only I _______________(ed)",
        pattern: ["if", "only", "i"],
        translations: ["Если бы только я ______!"],
        examples: [
          "If only I won a prize! (Если бы только я выиграл приз!)",
          "If only I found money! (Если бы только я нашел деньги!)",
          "If only I got a job! (Если бы только я получил работу!)"
        ],
        id: "if-only-i-past-simple",
        hasVerb: false,
        hasName: false
      },
      {
        structure: "I wish I _________________(ed)",
        pattern: ["i", "wish", "i"],
        translations: ["Как бы я хотел, чтобы я ______!"],
        examples: [
          "I wish I traveled the world! (Как бы я хотел, чтобы я путешествовал по миру!)",
          "I wish I bought a house! (Как бы я хотел, чтобы я купил дом!)",
          "I wish I saved money! (Как бы я хотел, чтобы я копил деньги!)"
        ],
        id: "i-wish-i-past-simple",
        hasVerb: false,
        hasName: false
      }
    ],
    ...
    validateStructure: function(text, structure) {
      ...
      let minWords = 6; // По умолчанию для a1
      if (structure.id === "if-i-past-simple-i-would-base") {
        minWords = 5; // If + I + V2 + I + would + V1
      } else if (structure.id === "if-only-i-past-simple") {
        minWords = 4; // If + only + I + V2
      } else if (structure.id === "i-wish-i-past-simple") {
        minWords = 4; // I + wish + I + V2
      }
      ...
      if (structure.id === "what-would-you-do-if-you-past-simple") {
        console.log('Начало проверки структуры What would you do if you past simple');
        const expectedStart = ['what', 'would', 'you', 'do', 'if', 'you'];
        ...
        return true;
      } else if (structure.id === "if-i-past-simple-i-would-base") {
        ...
        console.log('Начало проверки структуры If only I past simple');
        const expectedStart = ['if', 'only', 'i'];
        for (let i = 0; i < expectedStart.length; i++) {
          if (wordIndex >= words.length || words[wordIndex] !== expectedStart[i]) {
            console.log(`Ожидалось "${expectedStart[i]}" на позиции ${wordIndex}, получено`, words[wordIndex] || 'ничего');
            return false;
        ...
        console.log('Начало проверки структуры I wish I past simple');
        const expectedStart = ['i', 'wish', 'i'];
        for (let i = 0; i < expectedStart.length; i++) {
          if (wordIndex >= words.length || words[wordIndex] !== expectedStart[i]) {
            console.log(`Ожидалось "${expectedStart[i]}" на позиции ${wordIndex}, получено`, words[wordIndex] || 'ничего');
            return false;
