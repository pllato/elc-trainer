(function() {
  addLesson({
    level: "intermediate",
    lesson: "lesson12",
    name: "Урок 12: Future Perfect with Will Have",
    structures: [
      {
        structure: "I/He/She/We/They/You will (not) have _____ V3 ______(ed)",
        pattern: [],
        translations: ["Я/Он/Она/Мы/Они/Вы (не) ______ к ______"],
        examples: [
          "We will have been married for 30 years. (Мы будем женаты 30 лет.)",
          "He will have finished the project by tomorrow. (Он закончит проект к завтрашнему дню.)",
          "They will not have studied a lot by next month. (Они не будут много учиться к следующему месяцу.)"
        ],
        id: "subject-will-not-have-past-participle",
        hasVerb: false,
        hasName: false
      }
    ],
    ...
    validateStructure: function(text, structure) {
      ...
      const minWords = 5; // Подлежащее + will + (not) + have + V3 + дополнение
      ...
      const validAdverbs = ['hard', 'harder', 'well', 'better', 'fast', 'slowly', 'quickly', 'carefully', 'a lot', 'much', 'by tomorrow', 'by next week', 'by next month', 'for 30 years', 'for a long time'];
      ...
      if (structure.id === "subject-will-not-have-past-participle") {
        console.log('Начало проверки структуры Subject will (not) have past participle');
        // Проверяем подлежащее
        if (!['i', 'he', 'she', 'we', 'they', 'you'].includes(words[wordIndex])) {
          console.log('Ожидалось "i/he/she/we/they/you" на позиции', wordIndex, ', получено', words[wordIndex] || 'ничего');
          return false;
        }
        wordIndex++;
        ...
        while (wordIndex < words.length) {
          ...
          // Разрешаем предлоги и числительные в дополнении
          if (excludedWords.includes(word) && !validAdverbs.includes(word) && !['for', 'by', 'a', 'the', '30', 'years'].includes(word)) {
